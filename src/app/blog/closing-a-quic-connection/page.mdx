...
Closing a QUIC connection without losing any data however, is not as straight forward as you might like. I'm going to discuss this using the [Quinn API](https://docs.rs/quinn/latest/quinn/), but it applies to any QUIC version really.
...
5. The *receiver* may optionally close the  now. If so it should use [Endpoint::wait_idle](https://docs.rs/quinn/latest/quinn/struct.Endpoint.html#method.wait_idle) first to give the CONNECTION_CLOSE frame a chance to be re-sent if it did get lost.
