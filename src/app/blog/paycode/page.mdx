...
![Paycode toll booth deployment](/blog/paycode/paycode.png)
...
There are plenty of ways to solve connectivity issues: you can add a server to the local WiFi network, use a classic HTTP server, or run an [MQTT] broker. But these server-based approaches come with costs. More on-site hardware means more technicians to service that hardware. And the original problem never fully goes away — the single point of failure just shifts from the cloud to the local server.
