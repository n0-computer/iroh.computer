...
For a typical initial [`ClientHello`][clienthello], the server receives 1200 bytes (the initial packet is padded to 1200 bytes), so it is allowed to send 3600 bytes.
...
Blog posts like this an the 0-rtt blog post show off rarely used features of iroh, but in general if you use iroh in the most straightforward way possible you will already get excellent performance.
