...
But iroh is **not** sending certificate chains, but just a single Ed25519 public key in the [raw public keys in TLS][rfc7250] extension. An iroh server response is around 1 KiB today, well below 3600 bytes. So can we just forget about the topic?

Mostly, but... In most cases, address validation tokens are not that useful for iroh. But there is one user visible case where they make a difference: 0-rtt.
