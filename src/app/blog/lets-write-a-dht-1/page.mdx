...
In principle, values in a DHT can be of arbitrary size. But there are various reasons for wanting to keep the values small.

First, we want requests to store and look up data to be small for efficiency. Ideally a storage or lookup request should fit into a single network packet even with the inevitable framing overhead. In QUIC, the minimum MTU (maximum transmission unit) is specified as 1200 bytes. So if a request consisting of key, value, and some overhead fits into 1200 bytes, a request or response will be sent as a single non-fragmented UDP packet.

Second, we rely on arbitrary nodes on the network to store data for us without being in any way compensated for it. So the values need to be small so that a small DHT node can store many of them. If the value size were unlimited, people could and would abuse the DHT for storing actual data, which would put a lot of load on DHT nodes and make it extremely unlikely that people would run DHT nodes without being compensated for it.
...
The metric chosen by Kademlia is the XOR metric: the distance between two keys `a` and `b` is simply the bitwise XOR of the keys. This is absurdly cheap to compute and fulfills all the metric axioms. It also helps with sparse routing tables, as we will learn later.
...
<Note>
It would be easily possible for a modern machine to keep the entire set of known node IDs in memory even for very large DHTs. But you have to remember that routing is just a part of the work of the DHT, and the DHT process should be cheap enough in terms of memory and storage that you can run it in the background without slowing down the entire system.
</Note>
...
But since this is no longer the case, the lookup process needs to be iterative. We first use our local knowledge to find good candidates to ask, then ask them for the `k` closest nodes to our target `x` that they know. We then ask some of the resulting nodes the same question again, until we can no longer make an improvement. Basically, we do a greedy downhill search until we arrive at a minimum, which due to the power-law distribution of the nodes is hopefully a global minimum. There are some intricacies to this iterative algorithm. For example, we always ask multiple nodes at each stage while keeping the parallelism limited to avoid having too many concurrent connections. Fortunately, these complexities don't leak into the protocol. All we need is a way to ask a node for the `k` closest nodes to some key `x` according to its local knowledge.

<Note>Due to the limited size of the routing table and the low cost of computing the distance, this query is extremely cheap for a node to answer. Both the query and the response are small enough to fit into an unfragmented UDP packet.</Note>
...
For the key-value store part of the DHT RPC protocol, we want to keep things extremely minimalistic. So we need a way to set and get values.
...
So let's take a look at the RPC annotations. `Set` is a normal RPC call with a single answer message of type `SetResponse`, which contains some information about whether the set was successful and if not, why not. GetAll might return many values, so we use a response stream of `Value`s. The `#[rpc_requests(message = RpcMessage)]` annotation is to turn this enum into an irpc rpc protocol and to define a corresponding message enum. For details, see this [blog post about irpc].

GetResponse is still missing. It's an enum describing whether the set succeeded, and if not, why it failed. You might wonder why we don't use a `Result<(), SetError>`: you could do that, but you have to be aware that serializing detailed errors is sometimes a big pain, and also the exact details of the failure like the stack trace are nobody's business. We give you a very rough idea of why the request failed so you can decide whether to try again or not. For example, `ErrFull` means you might try a bit later; `ErrInvalid` means that there is something wrong with the value, e.g., signature error, and there is no point in trying again.
...
Note that for a mem transport, there is no way for the callee to check the requester node ID. For the RPC protocol, the mem transport is used in simulations where we trust the caller. When the request comes in via an iroh connection, we can do a quick check that the requester node ID is the remote node ID of the connection.
