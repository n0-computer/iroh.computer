...
the NAT router creates a mapping and sends the datagram from external address of it's choosing.
...
[^addr]: Technically we are dealing with *socket addresses*, which on IPv4 is indeed an IP address and port,
   ...
   so it is easier to think of of an IP & port 2-tuple.
   So naming this *address* is a bit a handwavy term,
   though sufficient to understand the needed logic.

For a Destination Endpoint Indenpendent mapping the mapping is straight forward:
...
To send a response the server would send a datagram addressed to this observed source address.
...
As part of the standardisation around audio-video calls in the form of SIP and WebRTC,
