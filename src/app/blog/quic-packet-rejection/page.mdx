...
export const post = {
  ...
  date: '2026-02-23',
  title: 'How QUIC allows rejecting garbage packets',
  description:
...
# How noq rejects garbage packets

Permissionless peer to peer systems frequently need to remain open to the world at least to some degree so that peers can join. But an endpoint that is open to the world is vulnerable to accidental or malicious spamming with random or even carefully crafted traffic.

This blog post explains how QUIC, and in particular our QUIC implementation noq, deals with various malicious packets.

At the wire level, disregarding relays and custom transports for a bit, QUIC is using UDP datagrams. Noq is reading individual datagrams from the operating system and then interprets them as higher level QUIC primitives, connections and streams.
...
## Iroh filtering and rate limiting API

![Connection filter pipeline](/blog/quic-packet-rejection/connection-filter.svg)
