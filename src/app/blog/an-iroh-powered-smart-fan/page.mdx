...
export const post = {
  ...
  description: 'Putting iroh on a bare ESP32 to build a humidity-driven smart fan you can check on from anywhere.',
}
...
If you live in Europe you are probably suffering from the heat wave. So let's do something against this, using iroh.

The previous ESP32 examples were just an echo protocol. But that is not what ESP32s are typically used for. You use an ESP32 as a cheap means to read sensors and drive actuators.

So we are going to write a very simple end to end example using an ESP32 to measure temperature and control a fan.

As the base we are going to use an esp32-WROOM devkit with 4 MiB of PSRAM, so we can use full iroh including a relay connection and remote control it from anywhere in the world. You can also use a M5StickC-Plus2.

<Note>
If all you have is an ESP32 without PSRAM, you can still use iroh. But you need to disable the relay connection and tweak QUIC buffers so we don't run out of memory. Use an appropriate example from the examples project as base.
</Note>
...
Let's try it out once before we do modifications. 'cargo run` on the server project will search for an esp32 connected via USB and flash it. So we just connect our esp32 with an USB cable.
...
We need to tell the esp32 how to connect to WLAN. In the example we just use another environment variable `WIFI_CONFIG=<access point name>:<password>`. Set this to your local WLAN.

You can do a single `export WIFI_CONFIG=<access point name>:<password>` so you don't have to pass it every single time.
...
To solve that the example has a little addition to the build.rs file that generates IROH_SECRET during build unless it is set via an environment variable.

That way we get a stable endpoint id after each flash and can reboot the device as many times as we want.

## Startup

On startup the device will try to connect to the WIFI using the given credentials. If that doesn't work it will hang. This happens before any iroh endpoint setup.
...
<Note>
We are using a tracing to log bridge, so we get rather verbose log messages.
</Note>
...
If you really want to shut it down, unplug it or delete the flash.
...
We will define the protocol in a separate crate, since it will be used by both the client and the esp32 itself.
...
Maybe we still want a simple way to check that the endpoint is up. We could of course add a ping endpoint to the irpc protocol, but we can also just keep supporting the echo protocol.
...
The CLI tool is nice for debugging, but what we really want is a GUI to show the temperature and later to control the fan. We could write a native GUI using dioxus that works on all major platforms. But who wants to install an app for this? So let's do a WASM gui that runs in the browser.
...
At this point we have a remote accessible temperature and humidity meter. But we want a smart fan. So let's add an output. We are going to use a 5V desktop computer fan like for example the [noctura nf-a14-5v]. This connects to the +5V and GND rail and has a single PWM control wire to switch or throttle the fan.

If you don't have such a fan, you can also just wire up a LED with a 300 Ω resistor, or wire up a relay to control a household fan. It's more fun with a real fan though!
...
We now want the ability to set the threshold above which the fan starts to work. But we still want the ability to have the smart fan GUI hosted on a public website. We could rely on the endpoint id being secret, but that is not a good idea. If we use discovery, data is published for the endpoint id on dns.iroh.link. Also, we might want to retain the ability to share a readonly view of the fan state.
...
At this point we have a bunch of code and a breadboard with some sensors wired up. But let's see if we can make this into a real product that we can put on our desk without it looking like an electronics lab.

# What else can you do

There are a wide variety of input and output devices available that work out of the box with an esp32. For input, there is everything from simple temperature and light sensors to precise CO2 and VOC sensors, accelerometers, GPS, microphones, even various cameras. The ESP32 can configure the GPIOs as analog digital converters, so you can easily use simple analog sensors such as potentiometers, photoresistors or switches.
...
[QAD]: /blog/qad
[ping protocol]: https://docs.rs/iroh-ping/latest/iroh_ping/
[irpc]: https://docs.rs/irpc/latest/irpc/
[noctura nf-a14-5v]: https://www.noctua.at/en/products/nf-a14-5v-pwm