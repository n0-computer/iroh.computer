...
As the base we are going to use an ESP32-WROVER devkit with 4 MiB of PSRAM, so we can use full iroh including a relay connection and remote control it from anywhere in the world. You can also use a M5StickC-Plus2, but you will have to adapt the GPIO pins.
...
Let's try it out once before we do modifications. `cargo run` on the server project will search for an ESP32 connected via USB and flash it. So we just connect our ESP32 with a USB cable.
