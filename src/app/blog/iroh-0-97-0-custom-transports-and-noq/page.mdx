...
**1. Changes to `Endpoint::builder` and `Endpoint::bind`**

`Endpoint::builder` and `Endpoint::bind` now take a preset argument, making the relay server configuration explicit. To get the same behaviour as before, simply replace `Endpoint::bind()` with `Endpoint::bind(presets::N0)`, and similarly with `Endpoint::builder()` becomes `Endpoint::builder(presets::N0)`:

**2. Changes to Endpoint closing**

Starting with this release, the `Endpoint` no longer attempts to close connections gracefully when dropped. To gracefully close the endpoint, always await `endpoint.close()` before dropping the last instance of an endpoint or terminating your application.
