...
Options include embedded WebPKI roots (the default), the operating system's roots, a custom list of roots, or any combination with additional extra roots. The `insecure_skip_verify` option is still available but is now gated behind the `test-utils` feature where it belongs.
...
The filter runs first, then the lookup service's own internal filtering runs second. This gives you fine-grained control over what gets published where.
