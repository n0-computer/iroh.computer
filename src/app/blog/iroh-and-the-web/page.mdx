...
export const post = {
  ...
  date: '2024-05-06',
  title: 'Iroh & the Web',
  description: 'announcing our plans to bringing iroh to the browser',
}
...
Today we have SDKs in rust, python, and swift via [`iroh-ffi`](https://github.com/n0-computer/iroh-ffi/).
Now we're excited to tackle the next environment: the browser.
...
Some of you keen network enthusiasts out there may be saying: Wait! Direct connections between machines is hard even outside browsers: There's NATs and firewalls getting in your way, too.
...
So far the protocol they used was a custom HTTP upgrade protocol that can't be established from a browser.
...
These are [widely supported](https://caniuse.com/?search=websockets) today.
...
There's many applications that depend only on `iroh-net` so far, and we're excited to see what these projects will use this for.
...
Even though that `iroh-net` is a key piece to the iroh stack - there's a lot more things in iroh!
...
In this phase we'll give crates the same treatment as we gave `iroh-net`:
...
As a rust developer, we'll want you to be able to use `iroh-net` in the browser just like you use it in other environments, as soon as possible.
No need to import another crate. Just `cargo build --target wasm32-unknown-unknown` and `wasm-bindgen`.
Full iroh API support in browsers will then follow later this year.

As a javascript developer, watch out for when the whole iroh API lands and you'll be able to use iroh documents and blobs directly in your browser.
