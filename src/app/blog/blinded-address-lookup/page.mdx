...
For every single place where the public key is exposed, we see a path forward for eliminating exposure. These rough sketches will still need vetting by a cryptography team once implemented to ensure that they are cryptographically sound. Please note, though, that while we have a proof-of-concept solution for one of the attack vectors (address lookup), ensuring that an `iroh` endpoint id can be used securely as a public key is not currently work we are planning to do in the near future.
