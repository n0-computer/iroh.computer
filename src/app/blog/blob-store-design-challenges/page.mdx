...
We've been hard at work with one of our protocols that you can pick off the shelf with iroh: [iroh-blobs]
You might've used iroh-blobs before if you tried out [`sendme`](https://www.iroh.computer/sendme), which is just a small CLI wrapper over iroh with iroh-blobs.

Put briefly, iroh-blobs helps transfer arbitrary data ("blobs") from one device to another via iroh in a way that allows these transfers to be interrupted, resumed and verified by content-addressing blobs using the BLAKE3 hashing function.

Iroh-blobs also allows you to exchange the backing storage it uses via *blob store* abstraction, so the actual hashes and blobs might be stored as files on disk, stored fully in-memory or even fully remote by utilizing some cloud storage.

Now, BLAKE3 hashed data and BLAKE3 verified streaming/bao are fully deterministic.
