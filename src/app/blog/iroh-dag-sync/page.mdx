...
So I have decided to just implement a number of useful traversals of varying level of complexity, but allow implementing additional deterministic traversals in rust. Of course this means that sync will have to be tailored to the needs of the application, and different applications will have different ALPNs
...
There are several parts missing to make this production ready.

The next step would be to write generators for realistic DAGs and perform benchmarks with them.

In addition, it needs a concept for tagging and garbage collection, very likely similar to the approach described in this [talk](https://www.youtube.com/watch?v=MCt3if9H64I).
