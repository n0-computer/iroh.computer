...
By default, iroh has a very minimalist set of primitives for content-addressed
data.
...
You might want to use different hash functions, for example because you have
a need for a hash function like [poseidon](https://eprint.iacr.org/2019/458.pdf)
that is more suitable to use in zero knowledge proofs. Or you have existing
...
Since the default iroh blobs store does not contain information about the 
...
The demo in [iroh-experiments] implements a simple unidirectional sync between
two nodes. You can think of it as [sendme], but for dags.
...
The above example sync a large dag using a single request/response interaction.
...
Here we specify the traversal to use as a [ron] expression, specifying a full
