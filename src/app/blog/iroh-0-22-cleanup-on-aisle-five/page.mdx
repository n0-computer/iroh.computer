...
We have a few subtle interactions happening in the downloader that we've untangled in this release. These interactions werearound around attempting to download content that you already had and attempting to download content from yourself.
...
Previously, however, we would wait until we knew the latency of a node's direct address before we attempted to send on that address. In a lot of scenarios, specifically the ones where we would need to hole-punch before being able to directly connect anyway, waiting for the `Ping`/`Pong` process to determine the latency was fine.

However, there are real scenarios where you might already know the public, direct address of the node you want to talk. In this case, you would just want the connection to be instant, since you know that no hole-punching is needed to establish a direct connection.

The changes in this release make that possible. If you know that a node has a direct, public address, when you supply that address using a `NodeAddr`, iroh will now immediately attempt to connect to that addresses.
