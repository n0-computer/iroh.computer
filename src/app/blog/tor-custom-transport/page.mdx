...
export const post = {
  ...
  date: '2026-01-22',
  title: 'Iroh TOR custom transports',
  description:
...
But what if I have additional ways to get data from one endpoint to another? Wouldn't it be nice if I could use them as well?
...
But we do not want to add additonal transports to the iroh codebase. That would make the code very complex with a maze of feature flags, add a lot of dependencies that most of our customers don't need, and it likely still would not cover all transports that people might want.
...
Wnen the iroh endpoint is created by the endpoint builder, it will interally create the custom endpoint for each custom transport, in addition to the UDP endpoint for direct connections and the TCP endpoint for relay connections.
...
The TOR daemon is available on all major platforms. Mac: `brew install tor`
...
The current example disables all transports except the custom transport. But in real world applications you will usually want to use either _multiple_ custom transports (e.g. bluetooth and tor for a private chat), or combine a custom transport with the built in transports IPV4, IPV6 and relay.
