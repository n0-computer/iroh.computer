...
There is a very comprehensive system with built-in address lookup via a directory built into Tor. It is somewhat similar to `iroh`, but with different goals. Where iroh gets the best possible performance via holepunching and direct connections, Tor hides the user location via its [onion routing] system to provide maximum privacy.
...
This data then is serialized using postcard and sent over the wire.
...
We have gone into some detail about the implementation. If you want even more detail, about the read side in particular, look at [the code].
...
The example sends a hardcoded text, but you could easily extend it to read from stdin to make a private chat app, or even use the transport together with a gossip based chat like we [develop in one of our workshops] to make a private group chat.
...
<Note>
If you care about privacy, you must disable existing transports and address lookup mechanisms and only enable transports and address lookup mechanisms that conform to your privacy requirements. Default iroh transports do provide encryption, but they expose some data about your 
</Note>
...
Each transport will have an idiomatic way to define addresses. E.g. bluetooth addresses are MAC addresses[^2], formatted as `00:1A:7D:DA:71:13`. Tor hidden service addresses are onion addresses, formatted as `vyj3sx7glv7ydiqn2giszacswg6stwlcn5tqtnwwdlh6adfza5if3gad.onion`.
...
[onion routing]: https://community.torproject.org/onion-services/overview/
...
[develop in one of our workshops]: https://github.com/n0-computer/iroh-workshop-39c3

[^1]: We might have an upper limit for the address size in the future. So far the largest we have seen is 32 bytes.
[^2]: On macOS, raw MAC addresses cannot be used for Bluetooth; UUIDs are required instead.