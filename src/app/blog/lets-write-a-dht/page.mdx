...
## Iroh connection tests

Memory based tests are great to check the behaviour of the routing algorithm. To test networking overhead, it is useful to have a test that runs in a single process but uses one iroh endpoint per node.

We will create an iroh endpoint per DHT node, then use a shared [static discovery provider](https://docs.rs/iroh/0.91.2/iroh/discovery/static_provider/struct.StaticProvider.html) to add dialing information for each node, and disable relay mode. In this test we will use a dedicated client pool per node that is actually backed by an iroh connection pool, just like you would do for a production DHT node.

Unsurprisingly, this test produces exactly the same results as the memory tests. It just takes a bit longer and is a great way to find issues in the underlying connection pool.
