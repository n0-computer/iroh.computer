...
connection via a relay server.  Once the connection to the relay server is established
...
in a datacenter.  It exists even though we want p2p connections,
because in today's internet we can not have direct connections without
...
[^1]: Almost: The QUIC handshake has to establish a TLS connection.
    ...
    text like any other TLS connection on the internet.  Yes, we know
    about ECH.  One thing at a time.
...
- Know which IP addresses it might be reachable on.  Some time we'll
  write this up in its own blog post, for now I'll just assume the
  endpoints know this.
...
which kind of packet this is: a QUIC datagram, or a holepunching datagram?  If
it is a QUIC packet it is passed onto the QUIC stack [^quicstack].  If
it is a holepunching datagram it needs to be handed by iroh itself, by a
...
[^greasing]: Since QUIC has released RFC 9287 which advocates
    "greasing" this bit: effectively toggling it randomly.  This is
...
of research, so we achieve reasonably decent results by now.
...
not.  What is worse: when holepunching succeeds, the iroh endpoint will
...
So you can make an educated guess on which path you'd like to send new
...
[^mpcongestion]: But hey!  Some of these paths share at least the
    ...
    are not.  Congestion control is still a research area,
...
- QUIC already has robust protection against sending too much data to
  unsuspecting hosts on the internet.
