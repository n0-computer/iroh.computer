...
The esp32 std environment, [esp-idf-sys], [esp-idf-hal], and [esp-idf-svc], is community provided, but very active and up to date. In contrast, all that I could find for the Pico is an old crate that provides a *very limited* (no WiFi) std environment for the Raspberry Pi Pico. [pico-std-rust] supports a std environment on Pi Pico RP2040. But still, better than nothing. We will have to add WiFi support and update it to RP2350, but it is a useful baseline.
