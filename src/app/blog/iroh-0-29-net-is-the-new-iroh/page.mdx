...
Setting up the docs protocol is a funny little beast. It’s a “meta protocol” that includes both the `iroh-blobs` and `iroh-gossip` protocols and so requires some involved set up. To make this a little bit cleaner, we’ll create an `Iroh` struct that looks a bit like what the `iroh::Node` looked like.
