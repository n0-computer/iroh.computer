...
Iroh v0.27.0 provided you with a Node that came bundled with the `iroh-docs`, `iroh-blobs`, and `iroh-gossip` protocols enabled. This also meant that iroh was not only a networking library, but also responsible for storage for these different protocols. Now, we've shifted our thinking. The iroh library provides you with an endpoint, a light framework for implementing your own protocols, and a router that combines that endpoint and those protocols (including the protocols we've also developed at number0).

That means we have no more `Node`  in iroh. If you rely on the node, we've added migration instructions for how to get the number 0 developed protocols into iroh, as well as custom protocols that you have developed yourself.
...
Note that, by default, we will bind to `0.0.0.0:0` and not enable any discovery, so please check out the build options for the `endpoint::Builder` in the [docs](https://docs.rs/iroh/latest/iroh/endpoint/struct.Builder.html) for more in-depth ways to configure the endpoint.

If you are converting straight from a default `iroh::Node` , make sure to include the `Builder::discovery_n0` option, which will enable the discovery mechanisms we had on by default.
...
Three important parts are needed when building a `Blobs` protocol handler to use with the `Router`: a local pool, a store, and a downloader.

The local pool manages all tasks spawned when working with `iroh-blobs`. It makes sure they are completed and closed once dropped. When building a `Downloader` and the `Blobs` handler, you will pass a handle to the `LocalPool`. Make sure you don’t accidentally drop the local pool early, as that will close all the tasks before you mean to.

The store is where the blobs are kept, this can be loaded from a file or be an in-memory store.
The downloader handles downloading blobs and connections concurrently from other nodes using the `iroh-blobs` protocol.
...
async fn main() -> Result<()> {
    ...
    // use [`iroh_blobs::net_protocol::Blobs::persistent`](https://docs.rs/iroh-blobs/latest/iroh_blobs/net_protocol/struct.Blobs.html#method.persistent) to load or create a persistent
    // blob store from a path
...
async fn main() -> Result<()> {
    ...
    // look at [`iroh_gossip::proto::topic::Config`](https://docs.rs/iroh-gossip/0.29.0/iroh_gossip/proto/topic/struct.Config.html)
...
Setting up the docs protocol is a funny little beast. It’s a “meta protocol” that includes both the `iroh-blobs` and `iroh-docs` protocols and so requires some involved set up. To make this a little bit cleaner, we’ll create an `Iroh` struct that looks a bit like what the `iroh::Node` looked like.

This example also shows how to load files from persistent storage for the blobs and docs stores, as well as the secret key and any default author.

Besides the protocols, you must also have `iroh-node-util`, for loading the secret key, and `quic-rpc`  for the rpc clients.
...
async fn main() -> Result<()> {
    ...
    builder = builder.accept(iroh_gossip::net::GOSSIP_ALPN, Arc::new(gossip.clone()));

    // add iroh-blobs
    let blobs = Blobs::memory().build(local_pool.handle(), builder.endpoint())?;
    builder = builder.accept(iroh_blobs::protocol::ALPN, blobs.clone());
...
It’s straightforward to add a custom protocol to the iroh `Router`. 

If you are looking for examples of how to create a client for your protocol using [quic-rpc](https://docs.rs/quic-rpc/0.17.0/quic_rpc), take a look at the [iroh-gossip repo](https://github.com/n0-computer/iroh-gossip/tree/main/src/rpc) for a simple example and the [iroh-blobs repo](https://github.com/n0-computer/iroh-gossip/tree/main/src/rpc) for a more complex one that includes streaming rpc (server side & bidirectional).

```rust
use iroh::{Endpoint, protocols::Router};

// build your protocol
let my_cool_protocol = MyCoolProtocol::new();
...
let router = Router::builder(endpoint)
    .accept(
        MyCoolProtocol::ALPN,
        // let the router know that your protocol is an iroh ProtocolHandler
        Arc::clone(&my_cool_protocol)
    )
    .spawn()
    .await?;
