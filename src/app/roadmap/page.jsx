  ...
  return (
    <BlankLayout>
        ...
        <div className='max-w-2xl mb-20'>
          <h1 className='text-3xl font-bold font-space-mono text-irohGray-700 mb-4'></h1>
  ...
  return (
    <div>
      <div style={{ marginLeft: '6.5rem' }} className='pr-5 relative flex gap-4 w-full'>
        <button className='cursor-pointer border-1 border-irohPurple-400 p-3 w-12 h-12 rounded-full' onClick={() => setCollapsed(!collapsed)}>
          {collapsed? <ChevronRight /> : <ChevronDown /> }
        </button>
        <button className='text-left cursor-pointer' onClick={() => setCollapsed(!collapsed)}>
          <h3 className='w-full text-xl leading-6 text-irohGray-600 font-space font-bold dark:text-irohGray-400'>{section}</h3>
          <p className='text-xs text-irohGray-400 dark:text-irohGray-600'>
    ...
    <ul className='pl-3 mt-2'>
      {Object.keys(data).map((taskName, i) => (
        <li key={i}>
          ...
          <span className='ml-2 leading-tight text-xs font-space-mono text-irohGray-500'>{taskName}</span>
        </li>
      ))}
    </ul>
  )
