...
```
// add this use statement to the top
use iroh::NodeId;
...
Here is where things get fun. We know how to create, join, and send and receive on a `Topic`. We also know how to get other to join that `Topic`. This has now created two different "roles" a node can have: a topic "creater" and a topic "joiner". One side "creates" the topic and the ticket, and the other side takes the ticket and uses it to join the topic and connect to the ticket creater.
