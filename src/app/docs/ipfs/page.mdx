...
[iroh.network](https://iroh.network) has configuration settings to automatically publish content it has to kubo. As of now publishing to kubo only works on 1Mb files. We plan to implement both large file publishing and fetching in the future.
