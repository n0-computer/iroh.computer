...
Iroh works with content-addressed _blobs_ of opaque data, which are often the bytes of a file. {{ className: 'lead' }}
...
Iroh uses collections as immutable, ordered lists of blobs. Collections themselves are blobs, serialized as a _hash sequence_ of one hash after another, with no separators or headers. Because all hashes in iroh are 32-byte BLAKE3 hashes, the byte length of a collection will always be a multiple of 32.
...
3. BLAKE3 chunks along 1Kib Boundaries, which means there will always bee exactly 32 hashes in a BLAKE3 incremental verification block.
