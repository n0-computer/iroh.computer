...
Iroh uses relay servers to support establishing direct connections, to speed up initial connection times, and to provide a fallback should direct conenctions between two endpoints fail or be impossible otherwise.
...
By chaning iroh's relay mode or relay map you can control the home relay the endpoint connects to, and by wrapping or writing your own `Discovery` service, you gain control over the relay URLs iroh can discover.
...
It is partly venture captial and partly founder backed (as in: founders have invested their own money).
Number 0 is healthy and has investors we actually think are a value-add.
We earn revenue from customers today through https://n0.computer/n0ps, and are re-launching an “AWS for iroh  protocols” in the next few weeks.
While it will be a service you can pay for, it won’t have special treatment beyond being convenient.
We rely on iroh remaining open source, and are committed to keeping it that was, including server-side code for relays and DNS discovery.
...
At least none of that is in iroh directly, but they're implemented as protocols on top, see e.g. [iroh-gossip](https://github.com/n0-computer/iroh-gossip/) which provides gossipsub-like functionality.
...
The iroh project was founded by developers who were deeply involved with libp2p, frustrated with its shortcomings and complexity and wanted to build something better.
