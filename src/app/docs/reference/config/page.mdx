...
### DERP nodes
DERP ("Designated Encrypted Relay for Packets") nodes are used to assist in holepunching and are identified by their url. We currently run two DERP nodes, one in North America and one in Europe. [Read more about DERP and holepunching.](/docs/layers/connections/holepunching) 

This is what the default configuration for the DERP nodes looks like:
```toml
[[derp_nodes]]
