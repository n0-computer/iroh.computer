...
To make composing protocols easier, iroh includes a "router" for composing togetner multiple protocols. The router implements the accept loop on your behalf, and routes incoming connections to the correct protocol based on the ALPN. We recommend using the router to compose protocols, as it makes it easier to add new protocols to your application.

We use the term "router" here because it mimics what an HTTP server would do with a URL-based router: It routes incoming connections to the correct protocol based on the ALPN.